...
def dashboard():
    ...
    try:
        ...
        # Количество ДЗ на проверке (ИСПРАВЛЕННЫЙ ЗАПРОС с course_teachers)
        cur.execute("""
            SELECT count(hs.id)
            ...
            JOIN school_edu.course_teachers ct ON ct.course_id = m.course_id
            WHERE hs.status = 'submitted' AND ct.teacher_id = %s
        """, (current_user.id,))
...
def homeworks():
    ...
    try:
        ...
        # GET-запрос: получаем список всех не проверенных ДЗ (ИСПРАВЛЕННЫЙ ЗАПРОС)
        cur.execute("""
            SELECT
            ...
            JOIN school_edu.course_teachers ct ON ct.course_id = c.id
